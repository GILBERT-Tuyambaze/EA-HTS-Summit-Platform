...
export type PartnerRecord = { id:string; company:string; category:string; contactPerson:string; email:string; phone:string; country:string; status:PartnerStatus; agreementStatus:AgreementStatus; logo?:string | null; createdAt:string };
...
const map=(x:Record<string,unknown>):PartnerRecord=>({id:String(x.id??''),company:String(x.company??x.organization??''),category:String(x.category??x.level??''),contactPerson:String(x.contact_person??''),email:String(x.email??''),phone:String(x.phone??''),country:String(x.country??''),status:(x.status==='Confirmed'||x.status==='Rejected'||x.status==='Pending'?x.status:'Pending') as PartnerStatus,agreementStatus:(x.agreement_status??'draft') as AgreementStatus,logo:x.logo?String(x.logo):null,createdAt:String(x.created_at??'')});
const payload=(x:Partial<PartnerInput>)=>({company:x.company,category:x.category,contact_person:x.contactPerson,email:x.email,phone:x.phone||null,country:x.country||null,status:x.status,agreement_status:x.agreementStatus,logo:x.logo||null});
