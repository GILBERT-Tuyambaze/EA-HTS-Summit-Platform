...
export default function AdminWorkspaceLayout({ title, description, children }: { title: string; description: string; children: ReactNode }) {
  ...
  return <div className="admin-root command-shell"><aside className="admin-sidebar command-sidebar"><button className="command-brand" onClick={() => navigate('/admin')}><span>IEEE</span><strong>EA-HTS</strong></button><p className="command-event">SUMMIT 2027</p><nav className="command-nav" aria-label="Admin navigation">{items.map(([label, path, Icon]) => <button key={path} className={`command-nav-item${current === path ? ' active' : ''}`} onClick={() => navigate(path)}><Icon size={18} /> {label}</button>)}</nav><div className="command-sidebar-bottom"><button className={`command-nav-item muted${current === '/admin/settings' ? ' active' : ''}`} onClick={() => navigate('/admin/settings')}><Settings size={18} /> Settings</button><button className="command-nav-item" onClick={() => { clearAdminSession(); window.location.assign('/'); }}><LogOut size={18} /> Sign out</button></div></aside><main className="admin-main command-main"><header className="command-topbar"><div className="event-switcher"><span className="event-mark">E</span><span><strong>EA Humanitarian Technology Summit</strong><small>Kigali · 27–29 Jan 2027</small></span></div><span className="admin-avatar">A</span></header><section className="workspace-heading admin-workspace-title"><div><p className="eyebrow admin-eyebrow">EA-HTS COMMAND CENTER</p><h1>{title}</h1><p>{description}</p></div><small>Last synced just now</small></section>{children}</main></div>;
}
