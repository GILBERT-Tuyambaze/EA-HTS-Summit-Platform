import { Globe, Mail, MapPin } from 'lucide-react';
import { useState } from 'react';
import { Link } from 'react-router-dom';
import CommandCenterModal from './CommandCenterModal';
import PartnerInquiryModal from './PartnerInquiryModal';
import { clearAdminSession, verifyAdminSession } from '../services/registrationService';

type InquiryType = 'partnership' | 'side-event' | 'challenge';

const Footer = () => {
  const [isInquiryOpen, setIsInquiryOpen] = useState(false);
  const [inquiryType, setInquiryType] = useState<InquiryType>('partnership');
  const [isCommandCenterOpen, setIsCommandCenterOpen] = useState(false);
  const [isCheckingSession, setIsCheckingSession] = useState(false);
  const [sessionMessage, setSessionMessage] = useState('');

  const handleOpenInquiry = (type: InquiryType) => {
    setInquiryType(type);
    setIsInquiryOpen(true);
  };

  const handleOpenCommandCenter = async () => {
    setIsCheckingSession(true);
    setSessionMessage('');

    try {
      await verifyAdminSession();
      window.location.assign('/admin');
    } catch (error) {
      clearAdminSession();
      setSessionMessage(
        error instanceof Error && error.message.toLowerCase().includes('expired')
          ? 'Your secure session has expired. Please sign in again.'
          : ''
      );
      setIsCommandCenterOpen(true);
    } finally {
      setIsCheckingSession(false);
    }
  };

  return (
    <>
      <footer className="footer">
        <div className="footer-container">
          <div className="footer-grid">
            <div className="footer-col footer-col-brand">
              <Link to="/" className="footer-brand-header footer-brand-link" aria-label="EA-HTS 2027 Home">
                <div className="footer-brand-badge">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" className="footer-brand-icon">
                    <circle cx="12" cy="12" r="9" />
                    <path d="M12 3v18" />
                    <path d="M3 12h18" />
                  </svg>
                </div>
                <div>
                  <div className="footer-brand-title">EA-HTS 2027</div>
                  <div className="footer-brand-subtitle">IEEE East Africa</div>
                </div>
              </Link>

              <p className="footer-brand-copy">
                The IEEE East African Humanitarian Technology Summit — a regional flagship platform uniting humanitarian, development, engineering, academic, and innovation communities across East Africa.
              </p>

              <div className="footer-social-row">
                <a
                  href="https://www.linkedin.com/company/ieee"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="IEEE on LinkedIn"
                  className="footer-social-button"
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="footer-social-svg">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                  </svg>
                </a>
                <a
                  href="https://x.com/EAHTS2027"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="EA-HTS 2027 on X (Twitter)"
                  className="footer-social-button"
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="footer-social-svg">
                    <path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z" />
                  </svg>
                </a>
                <a
                  href="https://www.instagram.com/eahts2027"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="EA-HTS 2027 on Instagram"
                  className="footer-social-button"
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="footer-social-svg">
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069ZM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0Zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324ZM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881Z" />
                  </svg>
                </a>
                <a
                  href="https://www.facebook.com/profile.php?id=61592386870922"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="EA-HTS 2027 on Facebook"
                  className="footer-social-button"
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="footer-social-svg">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073Z" />
                  </svg>
                </a>
              </div>
            </div>

            <div className="footer-col footer-col-links">
              <h3 className="footer-section-title">EXPLORE</h3>
              <div className="footer-link-list">
                <Link to="/about">About</Link>
                <Link to="/programme">Programme</Link>
                <Link to="/demo-village">Demo Village</Link>
                <Link to="/startup-awards">Startup Challenge</Link>
                <Link to="/side-events">Side Events</Link>
                <Link to="/partners">Partners</Link>
              </div>
            </div>

            <div className="footer-col footer-col-links">
              <h3 className="footer-section-title">GET INVOLVED</h3>
              <div className="footer-link-list">
                <Link to="/register">Register to attend</Link>
                <button
                  type="button"
                  className="footer-link-button"
                  onClick={() => handleOpenInquiry('partnership')}
                >
                  Become a partner
                </button>
                <button
                  type="button"
                  className="footer-link-button"
                  onClick={() => handleOpenInquiry('challenge')}
                >
                  Enter Startup Challenge
                </button>
                <button
                  type="button"
                  className="footer-link-button"
                  onClick={() => handleOpenInquiry('side-event')}
                >
                  Propose a side event
                </button>
                
              </div>
            </div>

            <div className="footer-col footer-col-contact">
              <h3 className="footer-section-title">CONTACT</h3>
              <div className="footer-contact-list">
                <a
                  href="mailto:ieeeahts27@gmail.com?subject=EA-HTS%202027%20Inquiry"
                  className="footer-contact-item"
                >
                  <Mail className="footer-contact-icon" />
                  <span>ieeeahts27@gmail.com</span>
                </a>
                <a
                  href="https://maps.google.com/?q=Kigali,+Rwanda"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="footer-contact-item"
                >
                  <MapPin className="footer-contact-icon" />
                  <span>Kigali, Rwanda</span>
                </a>
                <a
                  href="https://eac-hts-2027-do2s.vercel.app"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="footer-contact-item"
                >
                  <Globe className="footer-contact-icon" />
                  <span>eac-hts-2027-do2s.vercel.app</span>
                </a>
              </div>
            </div>
          </div>

          <div className="footer-bottom-bar">
            <span>© 2027 IEEE East African Humanitarian Technology Summit. All rights reserved.</span>
            <div className="footer-bottom-right">
              <span className="footer-micro-links">
                <Link to="/privacy-policy">Privacy</Link>
                <Link to="/terms-of-use">Terms</Link>
                <button
                  type="button"
                  className="footer-link-button"
                  onClick={() => handleOpenInquiry('partnership')}
                >
                  Contact
                </button>
              </span>
              
            </div>
          </div>
        </div>
      </footer>

      <PartnerInquiryModal
        open={isInquiryOpen}
        onClose={() => setIsInquiryOpen(false)}
        type={inquiryType}
      />

     
    </>
  );
};

export default Footer;
